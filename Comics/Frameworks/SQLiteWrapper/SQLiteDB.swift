...
class SQLiteDB: SQLiteBase {
	...
	override func open(dbPath: String = "", copyFile: Bool = true, inMemory: Bool = false) -> Bool {
		Debug.log("DB Open called with path: \(dbPath)")
		var path = ""
		if !inMemory {
			if dbPath.isEmpty {
		...
		Debug.log("Calling Super Open with path: \(path)")
	...
	override func closeDB() {
		if db != nil {
			...
			launchCount -= 1
			Debug.log("SQLiteDB - Launch count \(launchCount)")
			var clean = false
			if launchCount < 0 {
				clean = true
				launchCount = 500
			...
			// Clean DB
			Debug.log("SQLiteDB - Optimize DB")
			let sql = "VACUUM; ANALYZE"
			if CInt(execute(sql:sql)) != SQLITE_OK {
				Debug.log("SQLiteDB - Error cleaning DB")
			}
			super.closeDB()
		}
	}
