...
class CloudDB: NSObject {
	...
	func creaeZone(table: SQLTable, completion: @escaping ()->Void) {
		...
		privateDB.save(zone) {(_, error) in
			if let error = error {
				Debug.log("Error creating record zone for: \(table.table) - \(error.localizedDescription)")
			}
			completion()
		}
	}
	...
	func saveToCloud(row: SQLTable, dbOverride: DBType = .none) {
		...
		db.save(record) {(rec, error) in
			if let error = error {
				Debug.log("Error saving CloudKit data: \(error.localizedDescription)")
				return
			}
			...
			if let rec = rec {
				...
				Debug.log("Saved record successfully! ID - \(ckid)")
			}
		}
	}
	...
	func deleteFromCloud(row: SQLTable, dbOverride: DBType = .none) {
		...
		db.delete(withRecordID: ckid) { (rid, error) in
			if let error = error {
				Debug.log("Error deleting CloudKit record: \(error.localizedDescription)")
				return
			}
			Debug.log("Deleted record successfully! ID - \(rid!.recordName)")
		}
	}
