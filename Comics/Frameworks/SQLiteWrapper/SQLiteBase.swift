...
// If macOS, add app name to path
		...
		// Open the DB
		Debug.log("SQLiteDB Opening DB at: \(path ?? "no path")")
		let cpath = path.cString(using: String.Encoding.utf8)
		let error = sqlite3_open(cpath!, &db)
		if error != SQLITE_OK {
			// Open failed, close DB and fail
			Debug.log("SQLiteDB - failed to open DB!")
			sqlite3_close(db)
			return false
		}
		Debug.log("SQLiteDB opened!")
		return true
	}
	...
	private func prepare(sql: String, params: [Any]?) -> OpaquePointer? {
		...
		if params != nil {
			...
			for ndx in 1...cnt {
				//				Debug.log("Binding: \(params![ndx-1]) at Index: \(ndx)")
		...
		while result == SQLITE_ROW {
			...
			for index in 0..<columnCount {
				...
				if let val = getColumnValue(index: index, type: type, stmt: stmt) {
					//						Debug.log("Column type:\(type) with value:\(val)")
					row[key] = val
				}
			}
			rows.append(row)
	...
	private func getColumnType(index: CInt, stmt: OpaquePointer) -> CInt {
		...
		//		Debug.log("SQLiteDB - Got column type: \(buf)")
		if buf != nil {
			...
			//			Debug.log("SQLiteDB - Cleaned up column type: \(tmp)")
			...
			if let ptr = UnsafeRawPointer(sqlite3_column_text(stmt, index)) {
				...
				if var txt = String(validatingUTF8: uptr) {
					...
					if let dt = fmt.date(from: txt) {
						...
						Debug.log("String value: \(txt) but could not be converted to date!")
					}
				}
			}
